...
the prior 14/100 median under `@0.5.0`). The numbers, the test names, and the
caveats are all in [the generated report](validation-corpus.md).
...
- **Compliance** remains the weakest construct. Regime names (HIPAA, GDPR,
  SOC 2) and data-protection techniques in prompt text are not lawful handling.
  Ruleset `0.6.0` dropped bare `compliance|legal|encrypt|redact` after those
  matched safety-refusal copy and PDF tool lists on the public corpus; the
  catalog grade stays `author-intuition`.
- **Audit** and **Cost** still hold five patterns each. `0.6.0` tightened them
  against measured false positives (gift "budget", tool "rate limited",
  "TRACE every symbol", personality "accountability", "immutable security
  boundary"). **Read a `0` as "the rules did not find it", not as "you failed
  to write it"** — and a match only means the *wording* is present.
...
*Validation snapshot: package `0.6.1` (ruleset bump to `crewscore-hygiene@0.6.0` on main; cut as package when released), corpus report generated by `py scripts/validate_corpus.py`. The reproducible 356-prompt report supersedes the withdrawn 1,368-prompt study.*
